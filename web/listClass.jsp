...
<body>
	<div class="container">
		...
		<jsp:include page="header.jsp"></jsp:include>
		 ...
		 <form action="searchByName?method=searchClass" method="post" name="searchByName">
                    班级名称：<input type="text" name="searchName" value="<c:out value="${searchName}"></c:out>">
         <i class="icon-search"></i>&nbsp;<button type="submit" class="btn btn-success""  
         onclick="return checkSearch()"> 查找</button>&nbsp;&nbsp;
         <c:if test="${user.type==1 }">
         	<button type="submit" class="btn btn-success"><a href="addClass.jsp" style="color:white">添加</a></button>
         </c:if>
        </form>
		<table  class="table table-hover table-condensed text-center" style="margin-top:10px">
			...
			<c:forEach items="${pageBean.data}" var="cclass">
				<tr>
					<td>
						<c:out value="${cclass.classId}" />
					</td>
					<td>
						<c:out value="${cclass.cclasname}" />
					</td>
					<td>
						<c:out value="${cclass.teachername}" />
					</td>
					<td>
					<c:if test="${user.type==1}">
						<a href="deleteClass?classId=${cclass.classId}" class="btn btn-danger" onclick="return checkDelete()">删除</a>
						<a href="updateClass?method=updateUI&classId=${cclass.classId}" class="btn btn-primary">更新</a>
					</c:if>
			...
			<c:forEach items="${stuList}" var="cclass">
				<tr>
					<td>
						<c:out value="${cclass.classId}" />
					</td>
